...
- **クリップ ID は素材の内容指紋由来**(ファイル名・サイズ・長さ・作成時刻。パスや mtime は含めません): 素材を別ドライブ・別マシン・別 OS に移動しても ID が保たれ、メモ・選定・キャッシュを引き継げます
...
`project.json` と解析結果(文字起こし/シーン/VAD)は小さく再生成コストが高いので `VEH_PROJECT_DIR` に、サムネ・プロキシ(大容量)は `VEH_CACHE_DIR`、世代バックアップは `VEH_BACKUPS_DIR` に置き分けられます(デスクトップ版は後者2つをローカルへ自動分離)。**`VEH_PROJECT_DIR` だけをクラウド同期**すれば、GB 級のプロキシやバックアップのチャーンを避けつつメモ・文字起こしを複数マシンで共有できます。

**クラウド同期(OneDrive 等)への配慮**: 書き込みは**変更があった時だけ**・デバウンス+コアレスで間引き(頻繁なタイムスタンプ更新＝偽の競合を防ぐ)、大量ファイルを生成する**バックアップは同期フォルダの外**に出す、という[一般的なベストプラクティス](https://github.com/abraunegg/onedrive/blob/master/docs/usage.md)に沿っています。ただし**2台同時編集**の競合まではケアしないので、素材ドライブと同様「一度に1台」で使ってください。
...
- **自動アップデート**: 起動時に GitHub リリースの `latest.json` を確認し、新版があればダイアログで確認 → ワンクリックでダウンロード・適用・再起動(Tauri updater。署名付きアーティファクトを検証)。本体 UI は remote 配信で Tauri IPC が使えないため、更新導線は Rust 側に置いています。
- **保存先はいずれも変更可能**: 初回は起動画面から、以降はアプリの **「設定」メニュー**（データ/キャッシュの保存先を変更…）からいつでも変更できます（変更後は自動で再起動）。
